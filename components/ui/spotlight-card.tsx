...
const glowColorMap = {
  ...
  green:  { base: 145, spread: 55  },
  red:    { base: 0,   spread: 200 },
  orange: { base: 30,  spread: 200 },
};

/* Injected once — idempotent since all cards share the same CSS rules */
const GLOW_STYLES = `
  [data-glow]::before,
  [data-glow]::after {
    pointer-events: none;
  ...
  [data-glow]::before {
    background-image: radial-gradient(
      ...
      hsl(var(--hue, 145) calc(var(--saturation, 100) * 1%) calc(var(--lightness, 50) * 1%) / var(--border-spot-opacity, 1)),
      transparent 100%
    );
    filter: brightness(2);
  }
...
export function GlowCard({ children, className = "", glowColor = "green" }: GlowCardProps) {
  ...
  const inlineStyle: React.CSSProperties = {
    ...
    ["--size" as string]: "240",
    ["--outer" as string]: "1",
    ["--border-size" as string]: "calc(var(--border, 1.5) * 1px)",
    ["--spotlight-size" as string]: "calc(var(--size, 200) * 1px)",
    ["--hue" as string]: "calc(var(--base) + (var(--xp, 0) * var(--spread, 0)))",
    backgroundImage: `radial-gradient(
      var(--spotlight-size) var(--spotlight-size) at
      calc(var(--x, 0) * 1px) calc(var(--y, 0) * 1px),
      hsl(var(--hue, 145) 80% 65% / 0.05),
      transparent
    )`,
    ...
    border: "var(--border-size) solid rgba(0,0,0,0.09)",
    position: "relative",
    touchAction: "none",
  };
